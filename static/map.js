...
(function () {
  ...
  let awaitingNewPlace = false;
  let pinMarker = null;

  // --------------------------------------------------
  // Detect Folium map (single authority)
  // --------------------------------------------------
  function detectFoliumMap() {
    if (window.fmap) return;

    if (typeof L === "undefined") {
      return setTimeout(detectFoliumMap, 100);
    }

    for (const key in window) {
      if (key.startsWith("map_") && window[key] instanceof L.Map) {
        window.fmap = window[key];
        console.log("✅ fmap detected:", key);
  ...
  // --------------------------------------------------
  // Find FeatureGroup("marker") via LayerControl
  // --------------------------------------------------
  function findMarkerLayer() {
    for (const key in window) {
      const val = window[key];
      if (key.startsWith("layer_control_") && val?.overlays?.marker) {
        window.MarkerLayer = val.overlays.marker;
        console.log("📍 MarkerLayer resolved");
        return;
      }
    }
  }
  ...
  // Canvas marker icon
  // --------------------------------------------------
  window.makePrefixMarkerIcon = function (prefix, color = "#d9534f") {
  ...
  window.searchMap = async function () {
    ...
    if (!q || !window.fmap) return;

    const query = q.toLowerCase();
    let found = false;

    fmap.eachLayer(layer => {
      if (found) return;

      if (layer.getPopup?.()) {
        const content = layer.getPopup().getContent();
        if (String(content).toLowerCase().includes(query)) {
          const ll = layer.getLatLng?.() || layer.getBounds?.().getCenter();
          if (ll) {
            fmap.setView(ll, 17);
            layer.openPopup?.();
            found = true;
  ...
  // --------------------------------------------------
  // Add-place click handling
  // --------------------------------------------------
  function wireMapEvents() {
    fmap.on("click", async e => {
      if (!awaitingNewPlace) return;

      const prefix = prompt("Enter prefix:");
      if (!prefix) return;

      if (pinMarker) {
        MarkerLayer?.removeLayer(pinMarker);
        fmap.removeLayer(pinMarker);
      }

      const icon = makePrefixMarkerIcon(prefix);
      pinMarker = L.marker(e.latlng, { icon });

      (MarkerLayer || fmap).addLayer(pinMarker);

      try {
        const geo = await reverseGeocode(e.latlng.lat, e.latlng.lng);
        window.parent.postMessage(
          { type: "newPlaceCreated", prefix, ...geo, ...e.latlng },
          "*"
        );
        pinMarker.bindPopup(`<b>${prefix}</b><br>${geo.address}`).openPopup();
      } catch (err) {
        console.error(err);
      }

      awaitingNewPlace = false;
      fmap.getContainer().classList.remove("add-place-mode");
    });
  }
  ...
  window.addEventListener("message", e => {
    if (e.data?.type === "enableAddPlace") {
      awaitingNewPlace = true;
      fmap?.getContainer().classList.add("add-place-mode");
      alert("Click map to add a new place");
    }
